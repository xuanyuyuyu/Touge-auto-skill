...
7. Preserve clean formatting when writing into browser code editors. Prefer one atomic clipboard paste or an editor API/range replacement that inserts the entire prepared block without simulating Enter line by line. Do not use character-by-character typing for multiline code when the editor auto-indents, because it compounds indentation. If clipboard paste is unavailable, temporarily disable auto-indent or paste line-by-line while explicitly returning to column zero. Read the editor back after insertion and correct any staircase indentation before evaluation.
...
- After locating the failed rows, fields, elements, or checks, return to the authoritative task description and inspect the target effect image and any effect labels again. Do this even if they were inspected before the first attempt; the failed-test location tells which region of the target now deserves close inspection.
